...
OUTPUT_DIR = "./results/sinhsafe_llama_v2_temp"
...
# CRITICAL: Fix for Polyglots extended vocabulary mismatch
...
def load_and_split_data():
    global val_df_raw
    print("📊 Loading and splitting Excel datasets for V2...")
    ...
    test_df.to_csv("sinhsafe_test_set_v2.csv", index=False)
...
# --- 5. TRAINING (V2 REFINED + SUPERVISOR MODE) ---
trainer = SFTTrainer(
    model=model,
...
print("🚀 Starting SinLLaMA V2 Supervisor Training on GPU 1...")
trainer.train()

# --- 6. SUPERVISOR REPORT GENERATION ---
print("\n" + "="*60 + "\n🔄 GENERATING FINAL SUPERVISOR REPORT (V2)\n" + "="*60)
...
with open("final_llama_v2_report.txt", "w", encoding="utf-8") as f:
    f.write(report_content + final_block)
print("✅ V2 Supervisor Report saved to final_llama_v2_report.txt")
