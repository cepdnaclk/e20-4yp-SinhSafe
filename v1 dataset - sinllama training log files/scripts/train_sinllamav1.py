...
TOKENIZER_ID = "./SinLlama_Local" # Pointing local to bypass hub connectivity issues
OUTPUT_DIR = "./results/sinhsafe_llama_v1_temp"
...
# CRITICAL: Fix for Polyglots extended vocabulary mismatch (139,336 tokens)
...
# --- 4. DATA PREPARATION (Balanced & Split) ---
...
def load_and_split_data():
    global val_df_raw
    print("📊 Loading Excel datasets for V1...")
    ...
    test_df.to_csv("sinhsafe_test_set_v1.csv", index=False)
    
    # Handle Class Imbalance in Training only
    max_size = train_df['label'].value_counts().max()
    train_bal = pd.concat([
        train_df[train_df['label'] == lbl].sample(max_size, replace=True, random_state=42) 
        for lbl in ["Normal", "Offensive", "Harassment"]
...
trainer = SFTTrainer(
    ...
    args=SFTConfig(
        ...
        num_train_epochs=3,               # V1 standard
        learning_rate=2e-4,               # V1 standard
        bf16=True, 
        logging_steps=10,
        
        # --- EARLY STOPPING & STORAGE LOGIC ---
        eval_strategy="steps",
        eval_steps=50,
        save_strategy="steps",            
        save_steps=50,                    
        ...
        save_total_limit=1,               # Overwrites old checkpoints to save disk
...
print("🚀 Starting SinLLaMA V1 Supervisor Training on GPU 1...")
trainer.train()

# --- 6. SUPERVISOR REPORT GENERATION ---
print("\n" + "="*60 + "\n🔄 GENERATING FINAL SUPERVISOR REPORT (V1)\n" + "="*60)
...
print("\n🔍 Running Inference for Accuracy/F1...")
...
with open("final_llama_v1_report.txt", "w", encoding="utf-8") as f:
    f.write(report_content + final_block)
print("✅ V1 Supervisor Report saved to final_llama_v1_report.txt")
