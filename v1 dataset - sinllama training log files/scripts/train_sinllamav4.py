...
OUTPUT_DIR = "./results/sinhsafe_llama_v4_temp"
...
# CRITICAL fix for Polyglots extended vocabulary
...
def load_and_split_data():
    global val_df_raw
    print("📊 Loading and splitting Excel datasets for V4...")
...
# --- 5. TRAINING (V4 OPTIMIZED + REGULARIZED) ---
trainer = SFTTrainer(
    model=model,
    ...
    args=SFTConfig(
        ...
        gradient_accumulation_steps=8,    # Smooth gradients
        num_train_epochs=5,               # Ceiling for Early Stopping
        learning_rate=2e-4,               # LoRA Learning Rate
        lr_scheduler_type="cosine",
        optim="paged_adamw_32bit",
        neftune_noise_alpha=5.0,          # Regularization
        weight_decay=0.01,
        warmup_ratio=0.03,
        bf16=True, 
        logging_steps=10,
        ...
        save_total_limit=1,               # Disk protection
...
print("🚀 Starting SinhSafe V4 Training on GPU 1 (RTX 3090 Ti)...")
trainer.train()

# --- 6. SUPERVISOR REPORT GENERATION ---
print("\n" + "="*60 + "\n🔄 GENERATING FINAL SUPERVISOR REPORT (V4)\n" + "="*60)

# A. Loss History Table
...
# B. Final Inference Pass for Metrics
model.eval()
...
print("\n🔍 Running Validation Inference Pass...")
...
with open("final_llama_v4_report.txt", "w", encoding="utf-8") as f:
    f.write(report_content + final_block)
print("✅ V4 Supervisor Report saved to final_llama_v4_report.txt")