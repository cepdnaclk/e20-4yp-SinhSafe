...
def run_cv_training(df):
    ...
    best_overall_precision, champion_fold = 0.0, -1

    print(f"🚀 Starting 5-Fold CV on GPU 1 (Mixed Precision & Clean Data)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df['cleaned_text'], df['label'])):
        ...
        train_loader = DataLoader(train_ds, batch_size=TRAIN_ARGS["batch_size"], shuffle=True)
        val_loader = DataLoader(val_ds, batch_size=TRAIN_ARGS["batch_size"])
        ...
        best_fold_prec, epochs_data = 0.0, {}
    ...
    # --- SUPERVISOR REPORT ---
    print("\n" + "="*60 + "\n📊 1. DATA FOR LOSS CURVES\n" + "="*60)
    for f, losses in all_loss_histories.items():
        print(f"\n[ FOLD {f} LOSS HISTORY ]")
        for ep, data in sorted(losses.items()):
            print(f"Epoch {ep:<2} | Train: {data['train_loss']:.4f} | Eval: {data['eval_loss']:.4f}")
    ...
    print("\n" + "="*60 + "\n🎯 3. FINAL AVERAGES\n" + "="*60)
    print(f"AVG Accuracy: {np.mean([r['accuracy'] for r in fold_results]):.4f} | AVG Precision: {np.mean([r['precision'] for r in fold_results]):.4f}")
    print("="*60)

if __name__ == "__main__":
    print("\n" + "="*60 + "\n⚙️ CONFIG: DISK SAVING DISABLED | DATA CLEANING ENABLED\n" + "="*60)
    try:
        ...
        # --- THE FIX: Drop all NaN rows before splitting ---
        df = pd.concat([df_h, df_n, df_o], ignore_index=True)
        initial_count = len(df)
        df = df.dropna(subset=['cleaned_text', 'label'])
        df = df.sample(frac=1, random_state=42).reset_index(drop=True)
        
        print(f"✅ Loaded {len(df)} rows (Dropped {initial_count - len(df)} NaN rows)")
        run_cv_training(df)
